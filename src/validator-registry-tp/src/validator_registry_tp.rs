...
impl ValidatorRegistryTransactionHandler {
    fn _update_validator_state(&self,
        ...
        for x in 0..validator_map.entries.len() {

	
/*
        for entry_str in validator_map.entries.iter_mut() {
*/	
	    let mut entry_str = validator_map.entries.get_mut(x).unwrap();
		...
		let mut temp = serde_json::to_string(&entry)
                    .expect("Error converting Validator Registry Map Entry to string");
                *entry_str = temp;//.to_string();
