...
use base64;
use common::sgx_structs::{sgx_quote::SgxQuote, SgxStruct};
...
pub fn verify_signup_info(
    ...
    let public_key = match PKey::public_key_from_pem(report_public_key_pem.unwrap().as_bytes()) {
        Ok(key) => key,
        Err(_) => {
            return Err(ApplyError::InvalidTransaction(format!(
    ...
    let verification_report_dict = match verification_report_tmp_value.as_object() {
        Some(obj) => obj,
        None => {
            return Err(ApplyError::InvalidTransaction(format!(
    ...
    let epid_pseudonym_data = match verification_report_dict.get("epidPseudonym") {
        Some(data) => data,
        None => {
            return Err(ApplyError::InvalidTransaction(format!(
    ...
    let enclave_quote_base64 = match verification_report_dict.get("isvEnclaveQuoteBody") {
        Some(quote) => quote,
        None => {
            return Err(ApplyError::InvalidTransaction(format!(
    ...
    let valid_measurements_tmp =
        match get_config_setting(context, SAWTOOTH_POET_VALID_ENCLAVE_MEASUREMENTS) {
            Ok(setting) => setting,
            Err(_) => {
                return Err(ApplyError::InvalidTransaction(format!(
    ...
    let valid_basenames_tmp =
        match get_config_setting(context, SAWTOOTH_POET_VALID_ENCLAVE_BASENAMES) {
            Ok(setting) => setting,
            Err(_) => {
                return Err(ApplyError::InvalidTransaction(format!(
